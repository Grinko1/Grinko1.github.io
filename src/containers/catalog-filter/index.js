...
function CatalogFilter(props) {
  ...
  const callbacks = {
    // Сортировка
    onSort: useCallback(
      (sort) => store.actions[moduleName]?.setParams({ sort }),
      [store]
    ),
    // Поиск
    onSearch: useCallback(
      (query) => store.actions[moduleName]?.setParams({ query, page: 1 }),
      [store]
    ),
    // Сброс
    onReset: useCallback(
    ...
    onCategory: useCallback(
      (category) => store.actions[moduleName]?.setParams({ category, page: 1 }),
      [store]
    ),
  };
