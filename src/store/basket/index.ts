...
import { InitStateBasket } from "./types";
...
class BasketState extends StoreModule<Config, InitStateBasket> {
  initState(): InitStateBasket {
    return {
      list: [],
      sum: 0,
      amount: 0,
   ...
   */
  async addToBasket(_id: string, qtt: number) {
    let sum = 0;
    ...
    let exist = false;
    list = this.getState().list.map((item: Product) => {
      let result = item;
      if (item._id === _id) {
  ...
  removeFromBasket(_id: string) {
    ...
    const list = this.getState().list.filter((item: Product) => {
